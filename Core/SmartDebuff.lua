...
local sbb_w, sbb_h, sbb_upt, sbb_cur, sbb_nmax, sbb_n, sbb_per, sbb_dg, sbb_s, sbb_exp, sbb_gr, sbb_ach, sbb_x, sbb_xo, sbb_y;
...
function SmartDebuff_SetButtonBars(btn, unit, unitclass)
  if (unit) then -- and btn:IsVisible()
    ...
    if UnitIsPlayer(unit) then
      ...
      if not ns.IsSecretValue(sbb_nmax) and sbb_nmax > 0 then
    ...
    if (sbb_dg or not O.ShowHP) then sbb_n = 0; end
    btn.hp:SetStatusBarColor(sbb_col.r, sbb_col.g, sbb_col.b);
    if (O.ShowGradient) then
      btn.hp:SetStatusBarTexture("Interface\\TARGETINGFRAME\\UI-StatusBar")
    else
      btn.hp:SetStatusBarTexture("Interface\\TARGETINGFRAME\\UI-TargetingFrame-BarFill")
    end
    ...
    if (sbb_upt ~= 0 or sbb_dg or not O.ShowMana) then sbb_n = 0; end
    ...
    if (O.ShowGradient) then
      btn.mana:SetStatusBarTexture("Interface\\TARGETINGFRAME\\UI-StatusBar")
    else
      btn.mana:SetStatusBarTexture("Interface\\TARGETINGFRAME\\UI-TargetingFrame-BarFill")
    end
